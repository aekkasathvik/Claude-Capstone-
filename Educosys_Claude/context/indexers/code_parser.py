...
#we merging both the file extensions to get all the extensions that we want to index -first extenstion_to_language is a map so it is taking only keys of that map and then we are merging it with the text_extensions set to get all the extensions that we want to index
...
BLOCK_NODE_TYPES = {
  ...
  "function_item",    # Rust language uses this instead of function_definition
  "func_declaration", # Go  language uses this instead of function_definition
}

#@dataclass makes life easy by not letting us create a constructor and other methods for the class ParsedChunk. It automatically generates the __init__, __repr__, and other methods based on the class attributes.
@dataclass      
class ParsedChunk:
  name: str
